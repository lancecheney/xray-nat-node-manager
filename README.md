...
菜单 `7` 是只读的轻量单节点回程测试。输入 `浙江`、`上海`、`黑龙江` 这类不带“省/市”的省级简称后，脚本会依次测试当地中国电信、中国联通和中国移动目标；中文严格匹配，`浙江省` 或错字不会自动纠正。也可以直接输入一个 IPv4，只对这个地址运行一次测试，适合从中转机检查到落地机的线路；IP 模式不校验地址归属地、运营商或公网/私网属性。两种模式均为每跳一次探测，并显示典型国际线路名称、最后响应延迟、去重后的国际 AS 路径、跨运营商借道提示和保守的疑似绕路判断；国内接入和省网 ASN 不列入线路或路径显示。省级三网目标使用 [oneclickvirt/nt3 的 31 省级路由注册表](https://github.com/oneclickvirt/nt3/blob/main/model/snapshot/province-routes.json)。首次使用若缺少 NextTrace，脚本会从 [nxtrace/NTrace-core 官方发布](https://github.com/nxtrace/NTrace-core/releases)安装固定版本的 `nexttrace-tiny`，并用仓库内记录的官方发布文件 SHA-256 校验，避免 GitHub API 限流影响安装。

线路、借道与绕路均是基于可见跳点的推断，不等同于 BGP 或运营商结论。若目标运营商路径中出现其他运营商的已识别骨干 ASN（例如浙江联通的 `AS4809 → AS4837`），会单独显示“检测到借道”，不会被区域中转规则隐藏；这不自动等同于地理绕路。单个 GeoIP 国家标签不会直接判为绕路；只有不同于起点和终点的地区至少出现两个相邻公开跳点，并且 RTT 增幅合理时，才显示“疑似绕路”。新加坡→香港→上海这类常见亚洲区域中转会显示“经过香港，暂不判定为绕路”；香港到上海途中偶发一个 `1 ms 英国` 标签，只会作为证据不足的 GeoIP 标签忽略。隐藏路由或公开跳点不足时会明确显示无法判断。

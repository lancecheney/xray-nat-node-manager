...
- 安装完成会输出 3x-ui Agent 所需的主机、外部端口、基础路径和 HTTPS 设置；NAT 模式会同时明确显示“外部 TCP 端口 -> Agent 内部 TCP 端口”，3x-ui 主机设置必须填写外部公网端口。公网证书应使用标准 `verify` 校验，不再固定证书指纹。Token 需通过菜单中的敏感信息选项主动显示。

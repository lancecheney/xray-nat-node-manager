...
- 安装完成会按 `3x-ui 总面板 -> 节点 -> 添加主机` 的实际表单，输出节点主机地址、外部端口、基础路径和 HTTPS 设置。这里配置的是“节点主机”，Agent 只是该主机背后的连接接口。NAT 模式会同时显示“外部 TCP 端口 -> Agent 内部 TCP 端口”；3x-ui 的节点主机端口必须填写外部公网端口。公网证书应使用标准 `verify` 校验，不再固定证书指纹。Token 需通过菜单中的敏感信息选项主动显示。

...
- `3x-ui 总面板 -> 节点 -> 添加节点` 填写的是 Agent 管理连接：HTTPS、节点域名、Agent 外部公网 TCP 端口、基础路径、TLS 校验和 Token。选择入站后的“添加/编辑主机”才填写 VLESS/HY2/Reality 的公网地址与各自外部服务端口。脚本会分别输出这两套设置，并在 NAT 模式下分别显示对应的外部端口到内部端口映射。公网证书使用标准 `verify` 校验，不固定证书指纹。
